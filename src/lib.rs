...
/// and stored in the provided file.
///
/// # Example
///
/// ```no_run
...
/// # }
/// ```
pub fn new<P, R, S>(file: P, rng: &mut R, password: S) -> Result<(Vec<u8>, String), KeystoreError>
where
    P: AsRef<Path>,
    R: Rng + CryptoRng,
    S: AsRef<[u8]>,
{
    ...
    let uuid = encrypt_key(file, rng, pk.clone(), password)?;
    Ok((pk, uuid))
}
